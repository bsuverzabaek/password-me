...
function generatePassword(opts) {
    const selected = Object.keys(CHARSETS).filter((k) => opts[k]);
...
function calcStrength(opts) {
    const selected = Object.keys(CHARSETS).filter((k) => opts[k]);
    const poolSize = selected.reduce((sum, k) => sum + CHARSETS[k].length, 0);
    if (poolSize === 0)
        return { bits: 0, label: "", color: "#3e3e54" };
    const bits = Math.log2(poolSize) * opts.length;
    if (bits < 40)
        return { bits, label: "Very Weak", color: "#ef4444" };
    if (bits < 60)
        return { bits, label: "Weak", color: "#f97316" };
    if (bits < 80)
        return { bits, label: "Fair", color: "#eab308" };
    if (bits < 100)
        return { bits, label: "Strong", color: "#22c55e" };
...
function updateStrengthUI(opts) {
